...
const bookSearch= () => {
    ...
    .then(data => {
        if (data.numFound === 0) {
            ...
            result.innerHTML = `<h5 class="text-white p-2 bg-dark rounded fw-bold">Found ${data.docs.length} Results For "${data.q}"</h5>`
...
const countResult = (data) => {
    result.innerHTML = `<h5 class="text-white p-2 bg-primary rounded fw-bold">Books Available ${data.docs.length}</h5>`
};
...
const displayBook = data => {
    ...
    data.docs.forEach(item => {
        ...
        if (item.cover_i === undefined) {
            bookSrc = `https://www.creativindiecovers.com/wp-content/gallery/portfolio/greenweb.jpg`
